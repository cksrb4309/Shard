...
public class DamageTextVfxBatchEmitter : MonoBehaviour
{
    ...
    // 텍스트 이펙트(VFX / UI)에 전달되는 출력 요청용 데이터 구조체 모음
    // - TextEmitParams : 모든 텍스트 출력에 공통으로 사용되는 시각적 파라미터
    // - DefaultTextRequest : 문자열 기반 텍스트 출력 요청
    // - DamageTextRequest : 데미지(int) 기반 텍스트 출력 요청
