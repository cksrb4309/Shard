...
public class NearestAttackableSelector : MonoBehaviour
{
    ...
    //    return null;
    //}
    static Collider[] colliders_2= new Collider[300];
    public static IAttackable GetAttackable(Vector3 position, float maxRadius, List<IAttackable> ignoredList)
    {
        List<IAttackable> list = new List<IAttackable>();


        int count = Physics.OverlapSphereNonAlloc(position, maxRadius, colliders_2, instance.layerMask);

        if (count > 0)
        {
            for (int i = 0; i < count; i++)
            {
                IAttackable attackable = colliders_2[i].GetComponent<IAttackable>();
